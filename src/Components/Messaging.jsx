...
function Messaging() {
    ...
    const unsubscribe = onSnapshot(collection(db, 'users'), (snapshot) => {
      ...
      snapshot.forEach((doc) => {
        if (doc.id !== currentUserEmail) {
          loadedUsers.push({ email: doc.id, ...doc.data() });
        }
      });
  ...
  return (
    <div className="flex w-full max-w-4xl mx-auto h-[600px] bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      {/* Sidebar */}
            ...
            .map((u) => (
              ...
              >
                ...
                <div className="flex-1">
                  <span className={`block font-medium ${unreadCounts[u.email] > 0 ? 'font-bold text-black' : 'text-gray-700'}`}>
                    {u.username || u.email}
                  </span>
                </div>
      ...
      <div className="w-2/3 flex flex-col h-full">
        <div className="border-b border-gray-200 px-6 py-4 flex items-center">
          {selectedUser ? (
            ...
            <span className="font-bold text-lg text-gray-400">Select a user</span>
          )}
        </div>
